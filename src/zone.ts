...
export class ZoneLessModule {
    ...
    static forRoot(): ModuleWithProviders<ZoneLessModule> {
        return {
            ngModule: ZoneLessModule,
            providers: [
                AsyncNgZone,
...
export const withZoneLessModule = () => importProvidersFrom(ZoneLessModule.forRoot());
...
export class AsyncNgZone implements NgZone {
    ...
    schedulePromise(res: Promise<any>, label: string = '') {
        ...
        res.then(() => {
            ...
            this.runs--;
            if (this.nextError) clearTimeout(this.nextError);
            this.nextError = setTimeout(() => {
    ...
    protected newRun() {
        if (this.nextStable) clearTimeout(this.nextStable);
        this.isStable = false;
